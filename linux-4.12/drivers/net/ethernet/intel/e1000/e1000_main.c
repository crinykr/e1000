...
static void e1000_update_mng_vlan(struct e1000_adapter *adapter) {
	...
	if (!test_bit(vid, adapter->active_vlans)) {
		...
		if ((old_vid != (u16)E1000_MNG_VLAN_NONE) && (vid != old_vid) && !test_bit(old_vid, adapter->active_vlans))
...
static void e1000_power_down_phy(struct e1000_adapter *adapter) {
	 ...
	 */
	if (!adapter->wol && hw->mac_type >= e1000_82540 && hw->media_type == e1000_media_type_copper) {
		u16 mii_reg = 0;
...
void e1000_reset(struct e1000_adapter *adapter) {
	...
	if (legacy_pba_adjust) {
		...
		if (hw->mac_type == e1000_82547) {
			...
			adapter->tx_fifo_size = (E1000_PBA_40K - pba) << E1000_PBA_BYTES_SHIFT;
	 ...
	 */
	hwm = min(((pba << 10) * 9 / 10), ((pba << 10) - hw->max_frame_size));
	...
	if (hw->mac_type >= e1000_82544 && hw->autoneg == 1 && hw->autoneg_advertised == ADVERTISE_1000_FULL) {
		u32 ctrl = er32(CTRL);
...
const struct net_device_ops e1000_netdev_ops = {
	.ndo_open = e1000_open,
...
int e1000_open(struct net_device *netdev) {
	...
	err_req_irq: e1000_power_down_phy(adapter);
	e1000_free_all_rx_resources(adapter);
	err_setup_rx: e1000_free_all_tx_resources(adapter);
	err_setup_tx: e1000_reset(adapter);
...
int e1000_close(struct net_device *netdev) {
	 ...
	 */
	if ((hw->mng_cookie.status &
	E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT) && !test_bit(adapter->mng_vlan_id, adapter->active_vlans)) {
		e1000_vlan_rx_kill_vid(netdev, htons(ETH_P_8021Q), adapter->mng_vlan_id);
	}
...
static bool e1000_check_64k_bound(struct e1000_adapter *adapter, void *start, unsigned long len) {
	 ...
	 */
	if (hw->mac_type == e1000_82545 || hw->mac_type == e1000_ce4100 || hw->mac_type == e1000_82546) {
...
static int e1000_setup_tx_resources(struct e1000_adapter *adapter, struct e1000_tx_ring *txdr) {
	...
	if (!txdr->desc) {
		setup_tx_desc_die: vfree(txdr->buffer_info);
		return -ENOMEM;
	}
	...
	if (!e1000_check_64k_bound(adapter, txdr->desc, txdr->size)) {
		...
		if (!txdr->desc) {
			dma_free_coherent(&pdev->dev, txdr->size, olddesc, olddma);
...
static void e1000_configure_tx(struct e1000_adapter *adapter) {
	...
	default:
		...
		tdlen = adapter->tx_ring[0].count * sizeof(struct e1000_tx_desc);
		ew32(TDLEN, tdlen);
		ew32(TDBAH, (tdba >> 32));
	 ...
	 */
	if (hw->mac_type == e1000_82544 && hw->bus_type == e1000_bus_type_pcix)
...
static int e1000_setup_rx_resources(struct e1000_adapter *adapter, struct e1000_rx_ring *rxdr) {
	...
	if (!rxdr->desc) {
		setup_rx_desc_die: vfree(rxdr->buffer_info);
		return -ENOMEM;
	}
	...
	if (!e1000_check_64k_bound(adapter, rxdr->desc, rxdr->size)) {
		...
		/* Failed allocation, critical failure */
		if (!rxdr->desc) {
			dma_free_coherent(&pdev->dev, rxdr->size, olddesc, olddma);
...
static void e1000_setup_rctl(struct e1000_adapter *adapter) {
	...
	E1000_RCTL_RDMTS_HALF | (hw->mc_filter_type << E1000_RCTL_MO_SHIFT);
...
void e1000_unmap_and_free_tx_resource(struct e1000_adapter *adapter, struct e1000_tx_buffer *buffer_info) {
	if (buffer_info->dma) {
		if (buffer_info->mapped_as_page)
			dma_unmap_page(&adapter->pdev->dev, buffer_info->dma, buffer_info->length, DMA_TO_DEVICE);
		else
			dma_unmap_single(&adapter->pdev->dev, buffer_info->dma, buffer_info->length, DMA_TO_DEVICE);
		buffer_info->dma = 0;
	}
	if (buffer_info->skb) {
...
static unsigned int e1000_frag_len(const struct e1000_adapter *a) {
	return SKB_DATA_ALIGN(a->rx_buffer_len + E1000_HEADROOM) + SKB_DATA_ALIGN(sizeof(struct skb_shared_info));
}
...
static void e1000_clean_rx_ring(struct e1000_adapter *adapter, struct e1000_rx_ring *rx_ring) {
	...
	for (i = 0; i < rx_ring->count; i++) {
		...
		if (adapter->clean_rx == e1000_clean_rx_irq) {
			...
			if (buffer_info->dma)
				dma_unmap_page(&pdev->dev, buffer_info->dma, adapter->rx_buffer_len, DMA_FROM_DEVICE);
...
static int e1000_tx_map(struct e1000_adapter *adapter, struct e1000_tx_ring *tx_ring, struct sk_buff *skb, unsigned int first, unsigned int max_per_txd, unsigned int nr_frags, unsigned int mss) {
	...
	while (len) {
		 ...
		 */
		if (unlikely((hw->bus_type == e1000_bus_type_pcix) && (size > 2015) && count == 0))
			size = 2015;

		/* Workaround for potential 82544 hang in PCI-X.  Avoid
		 * terminating buffers within evenly-aligned dwords.
		 */
		if (unlikely(adapter->pcix_82544 && !((unsigned long) (skb->data + offset + size - 1) & 4) && size > 4))
			size -= 4;
	...
	for (f = 0; f < nr_frags; f++) {
		...
		while (len) {
			...
			if (unlikely(adapter->pcix_82544 && !(bufend & 4) && size > 4))
				size -= 4;
	...
	dma_error: dev_err(&pdev->dev, "TX DMA map failed\n");
	buffer_info->dma = 0;
	if (count)
		count--;
...
static int e1000_82547_fifo_workaround(struct e1000_adapter *adapter, struct sk_buff *skb) {
	...
	no_fifo_stall_required: adapter->tx_fifo_head += skb_fifo_len;
...
static netdev_tx_t e1000_xmit_frame(struct sk_buff *skb, struct net_device *netdev) {
	...
	if (unlikely((hw->mac_type == e1000_82547) && (e1000_82547_fifo_workaround(adapter, skb)))) {
		netif_stop_queue(netdev);
...
static int e1000_change_mtu(struct net_device *netdev, int new_mtu) {
	...
	if (!hw->tbi_compatibility_on && ((max_frame == (ETH_FRAME_LEN + ETH_FCS_LEN)) || (max_frame == MAXIMUM_ETHERNET_VLAN_SIZE)))
...
#define PHY_IDLE_ERROR_COUNT_MASK 0x00FF
	...
	if (hw->bad_tx_carr_stats_fd && adapter->link_duplex == FULL_DUPLEX) {
	...
	/* Phy Stats */
	if (hw->media_type == e1000_media_type_copper) {
		if ((adapter->link_speed == SPEED_1000) && (!e1000_read_phy_reg(hw, PHY_1000T_STATUS, &phy_tmp))) {
			phy_tmp &= PHY_IDLE_ERROR_COUNT_MASK;
			adapter->phy_stats.idle_errors += phy_tmp;
		}

		if ((hw->mac_type <= e1000_82546) && (hw->phy_type == e1000_phy_m88) && !e1000_read_phy_reg(hw, M88E1000_RX_ERR_CNTR, &phy_tmp))
 ...
 */
static void e1000_tbi_adjust_stats(struct e1000_hw *hw, struct e1000_hw_stats *stats, u32 frame_len, const u8 *mac_addr) {
	u64 carry_bit;
...
static bool e1000_tbi_should_accept(struct e1000_adapter *adapter, u8 status, u8 errors, u32 length, const u8 *data) {
	struct e1000_hw *hw = &adapter->hw;
...
static bool e1000_clean_jumbo_rx_irq(struct e1000_adapter *adapter, struct e1000_rx_ring *rx_ring, int *work_done, int work_to_do) {
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		if (unlikely((status & E1000_RXD_STAT_EOP) && (rx_desc->errors & E1000_RXD_ERR_FRAME_ERR_MASK))) {
			...
			if (e1000_tbi_should_accept(adapter, status, rx_desc->errors, length, mapped)) {
				length--;
...
#define rxtop rx_ring->rx_skb_top
		process_skb: if (!(status & E1000_RXD_STAT_EOP)) {
			...
			if (!rxtop) {
				...
				skb_fill_page_desc(rxtop, 0, buffer_info->rxbuf.page, 0, length);
			} else {
				/* this is the middle of a chain */
				skb_fill_page_desc(rxtop, skb_shinfo(rxtop)->nr_frags, buffer_info->rxbuf.page, 0, length);
			}
			...
			if (rxtop) {
				...
				if (length <= COPYBREAK_DEFAULT) {
					 ...
					 */
					skb_put(skb, length);
					e1000_rx_checksum(adapter, status | rx_desc->errors << 24, le16_to_cpu(rx_desc->csum), skb);
					...
					skb_fill_page_desc(skb, 0, p, 0, length);
					e1000_consume_page(buffer_info, skb, length);
				}
			}
		}
 ...
 */
static struct sk_buff *e1000_copybreak(struct e1000_adapter *adapter, struct e1000_rx_buffer *buffer_info, u32 length, const void *data) {
	struct sk_buff *skb;
...
static bool e1000_clean_rx_irq(struct e1000_adapter *adapter, struct e1000_rx_ring *rx_ring, int *work_done, int work_to_do) {
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		if (unlikely(rx_desc->errors & E1000_RXD_ERR_FRAME_ERR_MASK)) {
			if (e1000_tbi_should_accept(adapter, status, rx_desc->errors, length, data)) {
				length--;
			} else if (netdev->features & NETIF_F_RXALL) {
				goto process_skb;
			} else {
		...
		process_skb: total_rx_bytes += (length - 4); /* don't count FCS */
		total_rx_packets++;
...
static void e1000_alloc_jumbo_rx_buffers(struct e1000_adapter *adapter, struct e1000_rx_ring *rx_ring, int cleaned_count) {
	...
	while (cleaned_count--) {
		...
		if (!buffer_info->dma) {
			buffer_info->dma = dma_map_page(&pdev->dev, buffer_info->rxbuf.page, 0, adapter->rx_buffer_len, DMA_FROM_DEVICE);
			if (dma_mapping_error(&pdev->dev, buffer_info->dma)) {
				put_page(buffer_info->rxbuf.page);
				buffer_info->rxbuf.page = NULL;
				buffer_info->dma = 0;
...
static void e1000_alloc_rx_buffers(struct e1000_adapter *adapter, struct e1000_rx_ring *rx_ring, int cleaned_count) {
	...
	while (cleaned_count--) {
		...
		/* Fix for errata 23, can't cross 64kB boundary */
		if (!e1000_check_64k_bound(adapter, (void *) (unsigned long) buffer_info->dma, adapter->rx_buffer_len)) {
		...
		skip: rx_desc = E1000_RX_DESC(*rx_ring, i);
		rx_desc->buffer_addr = cpu_to_le64(buffer_info->dma);
...
static int e1000_mii_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd) {
	...
	case SIOCGMIIREG:
		spin_lock_irqsave(&adapter->stats_lock, flags);
		if (e1000_read_phy_reg(hw, data->reg_num & 0x1F, &data->val_out)) {
			spin_unlock_irqrestore(&adapter->stats_lock, flags);
			return -EIO;
		}
		spin_unlock_irqrestore(&adapter->stats_lock, flags);
	...
	case SIOCSMIIREG:
		...
		mii_reg = data->val_in;
		spin_lock_irqsave(&adapter->stats_lock, flags);
		if (e1000_write_phy_reg(hw, data->reg_num, mii_reg)) {
			spin_unlock_irqrestore(&adapter->stats_lock, flags);
			return -EIO;
		}
		spin_unlock_irqrestore(&adapter->stats_lock, flags);
			...
			case PHY_CTRL:
				...
				if (mii_reg & MII_CR_AUTO_NEG_EN) {
					...
					if (mii_reg & 0x40)
						...
						speed = SPEED_10;
					retval = e1000_set_spd_dplx(adapter, speed, ((mii_reg & 0x100) ? DUPLEX_FULL :
					DUPLEX_HALF));
					if (retval)
						return retval;
				}
...
static int e1000_vlan_rx_add_vid(struct net_device *netdev, __be16 proto, u16 vid) {
	...
	if ((hw->mng_cookie.status &
	E1000_MNG_DHCP_COOKIE_STATUS_VLAN_SUPPORT) && (vid == adapter->mng_vlan_id))
		return 0;
...
int e1000_set_spd_dplx(struct e1000_adapter *adapter, u32 spd, u8 dplx) {
	...
	if ((hw->media_type == e1000_media_type_fiber) && spd != SPEED_1000 && dplx != DUPLEX_FULL)
		goto err_inval;
